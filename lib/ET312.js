...
class ET312Base {
   ...
   * subclass.
   *
   * @returns {Promise} Promise that resolves on successful connect.
...
class ET312Serial extends ET312Base {
  /**
   * Creates an ET312 object with a serial port connection.
   *
   * @constructor
   * @param {String} port_addr Address of the serial port to create
   * @throws {Error} Throws if serial port cannot be opened.
   */
  constructor(port_addr) {
    super();
    if (port_addr === undefined) {
      throw new Error('ET312Serial requires a serial port address!');
    }
    if (typeof(port_addr) !== 'string') {
      throw new Error('ET312Serial requires a string as serial port address!');
    }
  ...
  open() {
    // Requiring the module here means it's not loaded until an ET312Serial
...
class ET312Emulator extends ET312Base {
  constructor() {
    super();
    this._rom = Buffer.alloc(256);
    this._ram = Buffer.alloc(1024);
    this._eeprom = Buffer.alloc(512);
  }
...
module.exports = {
  ET312Serial: ET312Serial,
  ET312Emulator: ET312Emulator
};
